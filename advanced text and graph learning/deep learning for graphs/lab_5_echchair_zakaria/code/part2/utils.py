...
def normalize_adjacency(A):
    ############## Task 9
    A = sp.csr_matrix(A)
    identity=sp.identity(A.shape[0], dtype='int8', format='dia')
    A_tilde = A+identity
    A_tilde_diag_v=A_tilde.sum(axis=1).squeeze()
    D_tilde=sp.spdiags(A_tilde_diag_v, 0, A_tilde_diag_v.size, A_tilde_diag_v.size)
    D_tilde.data=D_tilde.data**-0.5
    
    ##################
    A_normalized=D_tilde.dot(A_tilde.dot(D_tilde))
    ##################
    return A_normalized
