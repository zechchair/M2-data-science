...
class francais:
    ...
    def choices(a):
        ...
        l2 = ['Gaignebet', 'Yvon', "Charge de projets", 'Yvon.Gaignebet@emines.um6p.ma', 'D près des laboratoires']
            ...
            if any(item in jobs for item in list2):
                ...
                if 'oui' in d:
                    ...
                    if any(item in [ "logistique"] for item in list2):
                        l = l5
                    if any(item in [ "mécatronique"] for item in list2):
                        l = l2
                    francais.speak(l[1] + l[0]+l[2] + "de l'EMINES  "+" son bureau se  trouve dans le bloc   " + l[4]+" et voici son email qui s'affiche sur l'ecran si vous voulez le contacter ")
        ...
        elif francais.there_exists(["présente-moi","projet", "projets",]):
            print("qrcode")
...
while True:
    ...
    socket_send("dq,")
    #say("Cher Yvon Big Boss chers professeurs,Je me présente, Je suis EMINIA, robot d'assistance, réalisé dans le cadre des projets mécatroniques du deuxième année cycle d'ingénieurs à l EMINES, destiné à servir et guider les visiteurs de l'université mohammed 6 polytechnique Je vous laisse avec les étudiants qui ont été derrière mon étude, ma conception et ma fabrication, ils vont vous expliquer les différents étapes suivies tout au long de ce semestre selon le plan suivant :Question deplacement QR code,Je cède la parole à mes camarades ")
    socket_send("hide,")
    francais.start()
